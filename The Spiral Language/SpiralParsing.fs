...
let parse (settings: SpiralCompilerSettings) module_ = 
    ...
    let char_quoted s = (between (skipChar '\'') (skipChar '"' >>. spaces) (char_quoted_read (fun _ -> true)) |>> LitChar) s
    let string_quoted (s: CharStream<_>) = (between (skipChar '"') (skipChar '"' >>. spaces) (manyChars (char_quoted_read ((<>) '"'))) |>> LitString) s

    let inline string_raw_template str str' =
        skipString str >>. charsTillString str' true Int32.MaxValue .>> spaces
        |>> LitString

    let string_raw = string_raw_template "@\"" "\""
    let string_raw_triple = let x = "\"\"\"" in string_raw_template x x
    ...
    runParserOnString (between spaces eof expr) {ops=inbuilt_operators; semicolon_line= -1L} module_.name module_.code
