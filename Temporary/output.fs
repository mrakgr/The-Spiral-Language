module SpiralExample.Main
let cuda_kernels = """

extern "C" {
    
}
"""

type Tuple0 =
    struct
    val mem_0: int64
    val mem_1: int64
    val mem_2: int64
    new(arg_mem_0, arg_mem_1, arg_mem_2) = {mem_0 = arg_mem_0; mem_1 = arg_mem_1; mem_2 = arg_mem_2}
    end
and Env1 =
    struct
    val mem_0: (int64 [])
    val mem_1: Tuple0
    val mem_2: Tuple0
    new(arg_mem_0, arg_mem_1, arg_mem_2) = {mem_0 = arg_mem_0; mem_1 = arg_mem_1; mem_2 = arg_mem_2}
    end
and Env2 =
    struct
    val mem_0: int64
    val mem_1: int64
    new(arg_mem_0, arg_mem_1) = {mem_0 = arg_mem_0; mem_1 = arg_mem_1}
    end
and Tuple3 =
    struct
    val mem_0: Env2
    val mem_1: Env2
    val mem_2: Env2
    new(arg_mem_0, arg_mem_1, arg_mem_2) = {mem_0 = arg_mem_0; mem_1 = arg_mem_1; mem_2 = arg_mem_2}
    end
and Env4 =
    struct
    val mem_0: Env1
    val mem_1: Tuple3
    new(arg_mem_0, arg_mem_1) = {mem_0 = arg_mem_0; mem_1 = arg_mem_1}
    end
and Env5 =
    struct
    val mem_0: Env4
    new(arg_mem_0) = {mem_0 = arg_mem_0}
    end
let rec method_0((var_0: (int64 [])), (var_1: int64)): unit =
    let (var_2: bool) = (var_1 < 1L)
    if var_2 then
        let (var_3: bool) = (var_1 >= 0L)
        let (var_4: bool) = (var_3 = false)
        if var_4 then
            (failwith "Argument out of bounds.")
        else
            ()
        let (var_5: int64) = (var_1 * 6L)
        let (var_6: int64) = 0L
        method_1((var_0: (int64 [])), (var_5: int64), (var_6: int64))
        let (var_7: int64) = (var_1 + 1L)
        method_0((var_0: (int64 [])), (var_7: int64))
    else
        ()
and method_1((var_0: (int64 [])), (var_1: int64), (var_2: int64)): unit =
    let (var_3: bool) = (var_2 < 2L)
    if var_3 then
        let (var_4: bool) = (var_2 >= 0L)
        let (var_5: bool) = (var_4 = false)
        if var_5 then
            (failwith "Argument out of bounds.")
        else
            ()
        let (var_6: int64) = (var_2 * 3L)
        let (var_7: int64) = (var_1 + var_6)
        let (var_8: int64) = 0L
        method_2((var_0: (int64 [])), (var_7: int64), (var_8: int64))
        let (var_9: int64) = (var_2 + 1L)
        method_1((var_0: (int64 [])), (var_1: int64), (var_9: int64))
    else
        ()
and method_2((var_0: (int64 [])), (var_1: int64), (var_2: int64)): unit =
    let (var_3: bool) = (var_2 < 3L)
    if var_3 then
        let (var_4: bool) = (var_2 >= 0L)
        let (var_5: bool) = (var_4 = false)
        if var_5 then
            (failwith "Argument out of bounds.")
        else
            ()
        let (var_6: int64) = (var_1 + var_2)
        var_0.[int32 var_6] <- 1L
        let (var_7: int64) = (var_2 + 1L)
        method_2((var_0: (int64 [])), (var_1: int64), (var_7: int64))
    else
        ()
let (var_0: (int64 [])) = Array.zeroCreate<int64> (System.Convert.ToInt32(6L))
let (var_1: int64) = 0L
method_0((var_0: (int64 [])), (var_1: int64))
(Env5((Env4((Env1(var_0, Tuple0(0L, 0L, 0L), Tuple0(6L, 3L, 1L))), Tuple3((Env2(0L, 1L)), (Env2(0L, 2L)), (Env2(0L, 3L)))))))
