...
def signSGD(moduleList : list,learning_rate : float = 2 ** -7):
    """
    SignSGD ignores the magnitude of the gradient and moves in the direction of the sign by the specified learning rate.
    """
...
def create_nn_agent(vs_self,vs_one,neural,uniform_player,tabular): # self play NN
    ...
    def run(is_avg=False):
        ...
        signSGD([policy],learning_rate=2 ** -9)
        head *= head_decay

        # print(f"The l2 and l1 are {value.l2 / value.count} and {value.l1 / value.count}")
    ...
    for a in range(m):
        ...
        if (a + 1) % 30 == 0: 
            ...
            print(f'At iteration {a} The mean reward vs_self is {r.mean()}')
