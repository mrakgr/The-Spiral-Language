...
def belief_tabulate(state_probs : Tensor,head : Tensor,action_indices : Tensor,rewards : Tensor,env_probs : Tensor):
    # state_probs[batch_dim,state_dim]
    # head[action_dim*2,state_dim]
    # action_indices[batch_dim] : map (action_dim -> batch_dim)
    # rewards[batch_dim,1]
    # env_probs[batch_dim,1]
    ...
    def update_head(): # Weighted moving average update. Works well with probabilistic vectors and weighted updates that CFR requires.
        state_weights = env_probs * state_probs # [batch_dim,state_dim]
    ...
    def calculate():
        ...
        def state_probs_grad(): # The backprop derived rule.
            ...
            g = (prediction_errors * env_probs) * prediction_values_for_state # [batch_dim,state_dim]
...
def model_evaluate(value : Module,policy : Module,head : Head,is_update_head : bool,is_update_value : bool,
    ...
    def update(rewards_and_env_probs : Tensor):
        rewards_and_regret_probs = torch.from_numpy(rewards_and_env_probs).cuda().view(2,-1,1)
        rewards, env_probs = rewards_and_regret_probs[0,:,:], rewards_and_regret_probs[1,:,:]
        state_probs : Tensor = normed_square(value(value_data))
        update_head, calculate = belief_tabulate(state_probs.detach(),head.head.data,sample_indices,rewards,env_probs)
