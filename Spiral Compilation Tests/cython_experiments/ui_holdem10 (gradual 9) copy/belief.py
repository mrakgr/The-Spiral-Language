...
def belief_tabulate(state_probs : Tensor,head : Head,action_indices : Tensor,rewards : Tensor,regret_probs : Tensor):
    ...
    def value_grad(): # The backprop derived rule. Updates the head along the way.
        prediction_values_for_state = head.values[action_indices,:] # [batch_dim,state_dim]
        at_action_prediction_value = (state_probs * prediction_values_for_state).sum(-1,keepdim=True) # [batch_dim,1]
        prediction_errors = at_action_prediction_value - rewards
        prediction_errors_weighted = prediction_errors * regret_probs # [batch_dim,1]
        value_grad = prediction_errors_weighted * prediction_values_for_state # [batch_dim,state_dim]

        # Updates the head.
        head.weighted_values.index_add_(0,action_indices,-prediction_errors_weighted * state_probs)
        head.weights.index_add_(0,action_indices,regret_probs * state_probs)

        return value_grad, prediction_errors
...
def model_explore(
    ...
    def update(rewards_and_regret_probs : Tensor):
        ...
        value_grad, action_fun = belief_tabulate(state_probs.detach(),value_head,sample_indices,rewards,regret_probs)
        if is_update_value: 
            g,pred_ers = value_grad()
            if hasattr(value,'t'): 
