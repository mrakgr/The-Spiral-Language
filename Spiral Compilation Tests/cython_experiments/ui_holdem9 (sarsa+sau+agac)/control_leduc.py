...
import numpy as np
from belief import SignSGD,Head,model_evaluate,InfCube,Encoder,EncoderList
from torch.optim.swa_utils import AveragedModel

def neural_create_model(size,dim_head=2 ** 3,dim_emb=2 ** 6):
    # value = EncoderList(1,dim_head,dim_emb,Encoder(size.value,dim_head,dim_emb))
    # policy = EncoderList(1,dim_head,dim_emb,Encoder(size.policy,dim_head,dim_emb))
    value = Encoder(size.value,dim_head,dim_emb)
    policy = Encoder(size.policy,dim_head,dim_emb)
